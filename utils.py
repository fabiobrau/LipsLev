...
def generate_random_sentences(S,V,n,subset_z = None,k=1, alternative = None):
    ...
    for _ in range(k):
        ...
        out = [generate_sentence(s,z,u, V,k, alternative=alternative) for s,z,u in zip(out,positions,replacements)]
    return out
...
def collate_fn(data,to_id, tokenizer = char_tokenizer, pad = 286):
    ...
    unpadded = [tokenizer(S,to_id)[:pad] if len(S)>pad else tokenizer(S,to_id)[:pad] for S in SS]
    unpadded[0] = torch.nn.ConstantPad1d((0, pad - unpadded[0].shape[0]), 0)(unpadded[0])
...
def get_dataloaders(dataset,batch_size, word_to_id = None, word_level = False, n=13000,p_m = 0.8,min_length=10, max_length=40, pad = 286, valid_size = 1000):
    if dataset == 'numbers_letters':
        train_set = generate_numbers_and_letters(n,p_m,min_length=min_length, max_length=max_length)
        valid_set = generate_numbers_and_letters(200,p_m,min_length=min_length, max_length=max_length)
        test_set = generate_numbers_and_letters(200,p_m,min_length=min_length, max_length=max_length)
        ...
        if dataset in ['fake-news']:
            ...
            train_set = []
            for i in range(valid_size):
                valid_set.append({'sentence':dataset_dict['train'][-i]['sentence'], 'label':dataset_dict['train'][-i]['label']})
            for i in range(len(dataset_dict['train'])-valid_size):
                train_set.append({'sentence':dataset_dict['train'][i]['sentence'], 'label':dataset_dict['train'][i]['label']})
    if not word_level:
        if dataset == 'fake-news':
            char_to_id = get_char_to_id_list([train,test])
        else:
            char_to_id = get_char_to_id([train_set,valid_set,test_set])
        train_loader = DataLoader(train_set, batch_size = batch_size, shuffle = True, collate_fn = lambda x: collate_fn(x,char_to_id, pad=pad))
        valid_loader = DataLoader(valid_set, batch_size = batch_size, shuffle = False, collate_fn = lambda x: collate_fn(x,char_to_id, pad=pad))
        test_loader = DataLoader(test_set, batch_size = batch_size, shuffle = False, collate_fn = lambda x: collate_fn(x,char_to_id, pad=pad))
        return char_to_id, train_loader, valid_loader, test_loader
    else:
        train_loader = DataLoader(train_set, batch_size = batch_size, shuffle = True, collate_fn = lambda x: collate_fn(x,word_to_id, tokenizer=word_tokenizer, pad=pad))
        valid_loader = DataLoader(valid_set, batch_size = batch_size, shuffle = False, collate_fn = lambda x: collate_fn(x,word_to_id, tokenizer=word_tokenizer, pad=pad))
        test_loader = DataLoader(test_set, batch_size = batch_size, shuffle = False, collate_fn = lambda x: collate_fn(x,word_to_id, tokenizer=word_tokenizer, pad=pad))
...
Train/Test related
'''
def train(train_loader, net, optimizer, criterion, criterion_chars, epoch, device, grad_clip=float('inf'), lm_weight=0, masking_prob=0, lips_reg = 0,scheduler = None,tokenizer=None, pad=286):
    """ Perform single epoch of the training."""
    net.train()
    reg = None
    train_loss, correct, total = 0, 0, 0
    correct_chars, total_chars = 0, 0
    for idx, data_dict in tqdm(enumerate(train_loader)):
        text = data_dict[0]
        label = data_dict[1].to(device)
        if tokenizer is not None:
            inputs = tokenizer(data_dict[2], padding=True, return_tensors = 'pt')['input_ids'].to(device)
            inputs = torch.nn.functional.pad(inputs, (0,pad-inputs.shape[1]), "constant", 0)
        else:
        ...
        if lm_weight>0:
            loss = criterion(pred, label) + lm_weight*((mask==0)*criterion_chars(char_logits.transpose(1,2),inputs)).mean()
        else:
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(net.parameters(), max_norm=grad_clip, norm_type='inf')
        optimizer.step()
        ...
        if idx % 100 == 0 and idx > 0:
            ...
            m2 = ('Epoch: {}, Epoch iters: {} / {}\t'
                    'Loss: {:.04f}, Acc: {:.06f}, Acc chars: {:.06f}')
            print(m2.format(epoch, idx, len(train_loader), float(train_loss), acc, acc_chars))
    try:
...
def test(net, test_loader, device='cuda',tokenizer=None, pad=286):
    ...
    for (idx, data) in enumerate(test_loader):
        ...
        if tokenizer is not None:
            inputs = tokenizer(data[2], padding=True, return_tensors = 'pt')['input_ids'].to(device)
            inputs = torch.nn.functional.pad(inputs, (0,pad-inputs.shape[1]), "constant", 0)
        else:
            inputs = text.to(device)
        with torch.no_grad():
...
if __name__ == '__main__':
    ...
    SS = get_all_inserts(s,char_to_id) + get_all_deletes(s,char_to_id) + get_all_replacements(s,char_to_id)
    print(len(S), len(SS))
